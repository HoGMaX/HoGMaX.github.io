...
function setLanguage(lang){currentLanguage=lang==="uk"?"uk":"en";localStorage.setItem("artistic.language",currentLanguage);document.documentElement.lang=currentLanguage;if(languageToggle)languageToggle.querySelectorAll("[data-lang]").forEach(el=>el.classList.toggle("active",el.dataset.lang===currentLanguage));loadPrompt()}
...
function renderPrompt(prompt){
    ...
    const top=document.createElement("div");top.className="prompt-topbar";const back=document.createElement("a");back.href="./";back.className="prompt-back";back.textContent=UI[currentLanguage].back;top.appendChild(back);const category=document.createElement("div");category.className="prompt-category";category.textContent=prompt.category||"";top.appendChild(category);promptContent.appendChild(top);
    const header=document.createElement("header");header.className="prompt-header";const title=document.createElement("h1");title.className="prompt-title";title.textContent=t(prompt.title);const description=document.createElement("p");description.className="prompt-description";description.textContent=t(prompt.description);header.append(title,description);promptContent.appendChild(header);

    const images=imagesOf(prompt);const gallery=document.createElement("section");gallery.className="master-gallery";
    if(images.length){
        images.forEach(item=>{const pre=new Image();pre.src=srcOf(item)});
        const main=document.createElement("div");main.className="master-gallery-main";const image=document.createElement("img");image.className="prompt-hero-image";image.loading="eager";
        const controls=document.createElement("div");controls.className="gallery-controls";const prev=document.createElement("button");prev.className="gallery-arrow";prev.type="button";prev.textContent="←";prev.setAttribute("aria-label",UI[currentLanguage].previous);const counter=document.createElement("span");counter.className="gallery-counter";const next=document.createElement("button");next.className="gallery-arrow";next.type="button";next.textContent="→";next.setAttribute("aria-label",UI[currentLanguage].next);controls.append(prev,counter,next);main.append(image,controls);
        const thumbs=document.createElement("div");thumbs.className="master-gallery-thumbnails";const downloads=document.createElement("div");downloads.className="master-gallery-downloads";let active=0;let touchStartX=null;
        function renderGallery(animate=true){
            ...
            images.forEach((item,i)=>{const b=document.createElement("button");b.type="button";b.className=`gallery-thumbnail${i===active?" active":""}`;const im=document.createElement("img");im.src=srcOf(item);im.alt=nameOf(item,i);im.loading="lazy";b.appendChild(im);b.addEventListener("click",()=>{if(i!==active){active=i;renderGallery(true)}});thumbs.appendChild(b);const a=document.createElement("a");a.className="master-download";a.href=srcOf(item);a.download=nameOf(item,i);a.target="_blank";a.rel="noopener";a.textContent=`${UI[currentLanguage].download} ${String(i+1).padStart(2,"0")}`;downloads.appendChild(a)});
        }
    ...
    }else{const empty=document.createElement("div");empty.className="master-gallery-empty";empty.textContent=UI[currentLanguage].noImage;gallery.appendChild(empty)}
